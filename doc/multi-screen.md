...
```
main.ts
  └─ BasicCoordinatesAndSeasonsScreen            (Screen<BasicCoordinatesAndSeasonsModel, BasicCoordinatesAndSeasonsScreenView>)
       ├─ BasicCoordinatesAndSeasonsModel         owns all state
       └─ BasicCoordinatesAndSeasonsScreenView    owns all visuals
```
...
Mirror the structure of `src/basic-coordinates-and-seasons-screen/`:
...
existing `BasicCoordinatesAndSeasonsScreen.ts`.
...
const icon = new ScreenIcon(
  new Rectangle(0, 0, 548, 373, { fill: BasicCoordinatesAndSeasonsColors.accentColorProperty }),
