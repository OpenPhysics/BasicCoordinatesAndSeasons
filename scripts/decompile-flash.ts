 ...
 * Flash sources under `../Baseline/Astronomy/flash-animations/` (sibling Baseline repo), so the ported Basic Coordinates and Seasons sim can be
 ...
 *
 * Run with:
 *   npm run decompile                 # the five lab explorers/simulators → NAAP/decompiled/
 *   npm run decompile -- --all        # all BCS-relevant movies
 ...
 */
const BCS_TARGETS = [
  // The five published motion1 lab explorers/simulators (matched byte-for-byte to
  // their flashdev2 sources): two per coordinates screen, one for Seasons.
  "mapExplorer/mapExplorer010.swf", // tc_flat.swf — Terrestrial Coordinates: Flat Map Explorer
  "longLatDemo/longLatDemo014.swf", // tc_globe.swf — Terrestrial Coordinates: Globe Explorer
  "simpleFlatSkyMap/simpleFlatSkyMap007.swf", // cec_flat.swf — Celestial Equatorial Coordinates: Flat Sky Map
  "skyMap/skyMap028.swf", // cec_sky.swf — Celestial Equatorial Coordinates: Sky Explorer
  "eclipticSimulator/eclipticSimulator025.swf", // seasons_ecliptic.swf — Seasons and Ecliptic Simulator (lab shipped 024)
  // Supporting concept demos from the lab pages.
  "sexagesimalCalculator/sexagesimalCalculator001.swf",
  "daylightHoursExplorer/daylightHoursExplorer008-C.swf",
  "obliquity/obliquity004.swf",
  "latAngleDemo/latAngleDemo001.swf",
].map((p) => join(flashRoot, p));

/** The two simulators being ported, decompiled by default when no targets are given. */
const DEFAULT_TARGETS = BCS_TARGETS.slice(0, 5);
...
Usage:
  npm run decompile                  Decompile the five lab explorers/simulators → NAAP/decompiled/
  npm run decompile -- --all         Decompile all BCS-relevant movies
  npm run decompile -- <path>…       Decompile specific .swf / .fla / folder(s)

Flags:
  --all              Decompile the curated set of BCS-relevant SWFs
