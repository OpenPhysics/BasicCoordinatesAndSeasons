...
import { STAR_RADIUS } from "../../BasicCoordinatesAndSeasonsConstants.js";
...
export class CoordinateGuideNode extends Node {
  ...
  public constructor(projection: SkyProjection, options: CoordinateGuideNodeOptions) {
    ...
    const solid = (color: typeof BasicCoordinatesAndSeasonsColors.guideRaColorProperty, lineWidth: number): Path =>
      new Path(null, { stroke: color, lineWidth });
    const dashed = (color: typeof BasicCoordinatesAndSeasonsColors.guideRaColorProperty, lineWidth: number): Path =>
      new Path(null, { stroke: color, lineWidth, lineDash: DASH, opacity: 0.6 });

    const raFront = solid(BasicCoordinatesAndSeasonsColors.guideRaColorProperty, 1.5);
    const raBack = dashed(BasicCoordinatesAndSeasonsColors.guideRaColorProperty, 1.5);
    const decFront = solid(BasicCoordinatesAndSeasonsColors.guideDecColorProperty, 1.5);
    const decBack = dashed(BasicCoordinatesAndSeasonsColors.guideDecColorProperty, 1.5);
